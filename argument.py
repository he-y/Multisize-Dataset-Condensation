...
parser.add_argument('--imagenet_dir', default='./data', type=str)
...
parser.add_argument('--match',
                    type=str,
                    default='grad',
                    choices=['feat', 'grad', 'logit'],
                    help='feature ,gradient, or logit matching')
parser.add_argument('--metric',
                    type=str,
                    default='l1',
                    choices=['mse', 'l1', 'l1_mean', 'l2', 'cos'],
                    help='matching objective')
parser.add_argument('--after_softmax', type=str2bool, default=False, help='match logits after softmax or not')
parser.add_argument('--per_logit', type=str2bool, default=False, help='match every syn logits with all real logits')
...
parser.add_argument('--test_type', type=str, default='other', help='test type cx_cy')
...
parser.add_argument('--start_idx', type=int, default=0, help='which idx to start as intialization images')
...
parser.add_argument('--test_data_dir', type=str, default="", help='saved condensed data dir for testing')

# Dream
parser.add_argument('--interval', type=int, default=10, help='interval of sampling')

# Aim Tracker
parser.add_argument('--aim_repo', type=str, default=".aim", help='aim repository')
parser.add_argument('--use_aim', type=bool, default=True, help='whether to use aim or not')
parser.add_argument('--run_name', type=str, default="default_run", help='name of the run')
parser.add_argument('--exp_name', type=str, default="MDC", help='name of the experiment')
parser.add_argument('--load_checkpoint', action='store_true', help='resume from checkpoint without aim hash')
...
if args.run_name == "default_run":
    args.run_name = f"{args.dataset}_ipc{args.ipc}"
...
if args.dataset[:5] == 'cifar':
    ...
    if args.slct_type == "dsa":
        args.factor = 1
        args.mixup = "vanilla"  # very important to recover the baseline
    elif args.slct_type == "dc":
        args.factor = 1
        args.mixup = "vanilla"
        args.dsa = False
    elif args.slct_type == "mtt":
        args.factor = 1
    elif args.slct_type == "dream":
        args.batch_real = 128
...
if args.tag != '':
    ...
    args.run_name += f'_{args.tag}'

if args.ipc > 0:
    if args.slct_type == 'random':
        args.tag += f'_rand{args.ipc}'

    elif args.slct_type in ['idc', 'dream']:
        # Matching
        if args.slct_type == 'dream':
            args.tag += f'_dream'
            args.run_name += f'_dream'
        ...
        if args.adaptive_reg:
            ...
            if args.adaptive_class_wise:
                args.tag += f'_class_wise'
                args.run_name += f'_class_wise'
        ...
        if args.batch_syn_max != 128:
            ...
            args.run_name += f'_synmax{args.batch_syn_max}'

        args.tag += f'_{args.init}'
        args.tag += f'_ipc{args.ipc}'

        if args.start_idx != 0:
            args.tag += f'_idx_{args.start_idx}'
            args.run_name += f'_idx_{args.start_idx}'

        # For multi-processing (class partitioning)
        if args.nclass_sub > 0:
            args.tag += f'_{args.nclass_sub}'
            args.run_name +=  f'_{args.nclass_sub}'
        if args.phase >= 0:
            args.tag += f'_phase{args.phase}'
            args.run_name += f'_phase{args.phase}'
else:
...
# Result folder name
if args.test:
    args.save_dir = 'raid/results/test'
    print(f"Args tag: {args.tag}")
    print(f"run_name: {args.run_name}")
else:
    args.save_dir = f"raid/results/{datatag}/{modeltag}{args.tag}"
