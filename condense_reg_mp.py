...
class Synthesizer():
    ...
    def init(self, loader, init_type='noise'):
        ...
        if init_type == 'random':
            ...
            for c in range(self.nclass_sub):
                img, _ = loader.class_sample(self.subclass_list[c], self.ipc * self.factor**2, start_idx=args.start_idx)
...
def condense(args, logger, device='cuda'):
    ...
    for init_loop in range(n_iter):
        ...
        loss_list_per_class = [[] for i in range(args.nclass)] # should have shape (nclass, )
        freeze_ipc_list = [-1 for i in range(args.nclass)]
        for model_loop in range(args.inner_loop):
            ...
            for c in subclass_list:
                ...
                img_syn, lab_syn, sampled_indices = synset.sample(real_to_idx[c], max_size=args.batch_syn_max)
                ts.stamp("data")
        ...
        if (init_loop % it_log == 0) and (not args.test):
            logger(
                f"{utils.get_time()} (Iter {init_loop:3d}) loss: {loss_total/args.nclass_sub/args.inner_loop:.1f}")
        ...
        if condition1 and condition2:
            for c in subclass_list:
                if args.adaptive_class_wise:
                    ...
                    flattened_loss_list = torch.Tensor(loss_list_per_class[cls_from:cls_to]).reshape(-1, len(loss_list_per_class[c][0]))
