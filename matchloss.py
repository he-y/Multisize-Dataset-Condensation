...
def time_func(name, func, *args, **kwargs): # compute the time of the function
    start = time.time()
    out = func(*args, **kwargs)
    end = time.time()
    # print(name, (end - start)*10000)
    return out
...
def matchloss(args, img_real, img_syn, lab_real, lab_syn, model):
    ...
    if args.match == 'feat':
        ...
        # len(feat) == 1
        # feat_tg[0].shape = (64, 128, 4, 4)
        # feat[0].shape    = (40, 128, 4, 4)
        for i in range(len(feat)):
            # feat[0].mean(0).shape = (128, 4, 4)
