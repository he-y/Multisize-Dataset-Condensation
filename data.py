...
class ClassMemDataLoader():
    ...
    def class_sample(self, c, ipc=-1, start_idx=0):
        # note the `ipc` is after decoding
        if ipc > 0:
            indices = self.cls_idx[c][start_idx*ipc:(start_idx+1)*ipc]
        else:
            indices = next(self.class_sampler.samplers[c])
...
class ClassPartMemDataLoader(MultiEpochsDataLoader):
    """Class loader for ImageNet-10 with multi-processing.
